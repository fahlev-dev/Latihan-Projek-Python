...
class ContactsApp(App):
    ...
    def action_delete(self):
        contact_list = self.query_one(DataTable)
        row_key, _ = contact_list.coordinate_to_cell_key(
            contact_list.cursor_coordinate
        )
        
        def check_answer(accepted):
            if accepted and row_key:
                self.db.delete_contact(id=row_key.value)
                contact_list.remove_row(row_key)
        
        name = contact_list.get_row(row_key)[0]
        self.push_screen(
    ...
    def compose(self):
        yield Header()
        contact_list = DataTable(classes="contact-list")
        contact_list.focus()
        contact_list.add_columns("Name", "Phone", "Email")
        contact_list.cursor_type = "row"
        contact_list.zebra_stripes = True
        ...
        yield Horizontal(contact_list, buttons_panel)
        yield Footer()
